...
public class THC1_ctrl : MonoBehaviour {
    ...
    void Start ()
    {
        ...
        if (controller == null)
        {
            Debug.LogError("CharacterController not found on " + gameObject.name + ". Please add a CharacterController component.");
        }
    ...
    void Update ()
    {
        if (controller == null)
        {
            return; // Exit Update if CharacterController is missing
        }
